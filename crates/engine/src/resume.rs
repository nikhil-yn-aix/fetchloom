...
pub enum ResumeRung {
    /// An outboard tree is known for the expected digest, so the bytes on disk
    /// are verified by range and the transfer resumes from the first bad or
    /// missing chunk.
    Outboard,
    ...
    StrongValidator,
    /// Only a weak validator is available, so a mismatch quarantines.
    WeakValidator,
    /// No validator is available, so the transfer restarts from zero.
    NoValidator,
}
