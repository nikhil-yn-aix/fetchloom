...
/// The corpus entries whose rejection needs the destination volume or the
/// order extraction creates entries in, and which the reader therefore lists
/// without deciding.
...
/// The corpus entries whose declared totals stay within the default limits
/// on purpose, so their rejection is only reachable against an overridden
/// limit and is asserted in `extract.rs` rather than here.
