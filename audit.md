...
**Widened at `aa19f31`.** `crates/cli/src/main.rs` now builds a `CommandLinePolicy` in every command that touches a run (`main.rs:436,524,777,853`), where before nothing built one outside `crates/cli/tests/policy.rs`. Production code reads twelve of the trait's fifteen methods off it: `cache_directory`, `verification`, `durability`, `accepts`, `limits`, `concurrency`, `per_host`, `bandwidth`, `aggressive`, `adapts` and `io` all have a call site in `main.rs`, and `offline` has one in `run.rs`. `offline` was the last of the settings-shaped methods without one: the offline check read the resolved settings directly while every sibling went through the seam, which is the second way standards.md forbids, and it now takes a `&dyn Policy` and asks the seam. The behavior is unchanged and contracts.md:923 still holds -- an offline run refuses a network reference with `policy.offline` and exits 40 -- and `crates/cli/tests/policy.rs the_seam_is_what_refuses_a_network_reference_offline` asserts it against a policy that carries no settings at all, so only the seam can have answered. `credential`, `offer_credential` and `terms` still have no production caller outside `policy.rs`'s own tests, so `policy.credential_missing`, `credential.required`, `credential.offer` and `credential.declined` remain unreached in a run, as F8 already says. The commit's own description names what changed: `--concurrency`, `--per-host` and `--bandwidth` are resolved through the seam and given a per-host controller that adapts from what the cache recorded, which is the seam's first arbitrating caller and the thing this finding said was missing. What is not closed is candidate arbitration in the sense phase 7 needs -- choosing between two sources -- because there is still one source until phase 7 ships it. The settings half of this finding is closed; the arbitration half is confirmed still open and is phase 7's, not phase 6's.
...
by every run in `crates/cli/src/main.rs` rather than by tests alone, and twelve of
